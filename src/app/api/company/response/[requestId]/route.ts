...
export async function GET(_request: NextRequest, { params }: { params: { requestId: string } }) {
  try {
    ...
    const responseData = await CompanyResponseService.getResponseData(requestId, companyData.id);
...
export async function POST(request: NextRequest, { params }: { params: { requestId: string } }) {
  try {
    ...
    logger.info('Request body parsed', {
      ...
      selectedOrganismesCount: Array.isArray(body.selected_organismes)
        ? body.selected_organismes.length
        : 0,
      availableServicesCount: Array.isArray(body.services)
        ? body.services.filter((s: { is_available: boolean }) => s.is_available).length
        : 0,
    });
    ...
    logger.info('Company response created successfully', {
      ...
      servicesResponded: body.services.filter((s: { is_available: boolean }) => s.is_available)
        .length,
...
export async function PUT(request: NextRequest, { params }: { params: { requestId: string } }) {
  try {
    ...
    logger.info('Company response updated successfully in route', {
      ...
      servicesUpdated: body.services.filter((s: { is_available: boolean }) => s.is_available)
        .length,
      selectedOrganismesCount: body.selected_organismes?.length || 0,
    });
...
export async function DELETE(_request: NextRequest, { params }: { params: { requestId: string } }) {
  try {
    ...
    const response = await CompanyResponseService.deleteResponse(requestId, companyData.id);

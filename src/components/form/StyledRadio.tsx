...
export const StyledRadio = ({
  ...
  const component = (
    <div className={`relative ${className}`}>
      ...
      />
      <div
        className={`${sizeClasses[size]} rounded-full border-2 transition-all duration-200 cursor-pointer ${
  ...
  if (label) {
    return (
      <label className="flex items-center space-x-3 cursor-pointer group">
        {component}
        <span className={`text-sm font-medium text-gray-700 select-none ${labelClassName}`}>
          {label}
        </span>
      </label>
    );
